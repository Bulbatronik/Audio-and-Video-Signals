...
[x_v, Fs] = audioread('voiced.wav');% same sampling frequency
...
frame_spacing = floor(10e-3 * Fs);% hop size
...
for n=1:N
    ...
    % compute features
    f_v(n,1) = sum(abs(diff(frame>0))) / frame_length;% feature matrix v
    f_v(n,2) = sum(frame.^2);
end
...
for n=1:N
    ...
    % compute features
    f_u(n,1) = sum(abs(diff(frame>0)))/frame_length;% feature matrix u
    f_u(n,2) = sum(frame.^2);
end
...
f_db = [f_v; f_u];% database of features

% concatenate labels
y_db = [zeros(length(f_v), 1); ones(length(f_u), 1)];% labels
...
% loop over windows of x_v
for n=1:N % as before for the test set
...
for f_idx = 1:length(f_t)
    ...
    distance = sqrt(sum((f_db - f).^2, 2)); % check the entire DB
    
    % sort distances and select classes from K closest features
    [val, idx] = sort(distance);%from the shortest/closest
    classes = y_db(idx(1:3));% 3 labels which are closer tho the features
    
    % apply majority voting to estimate the class of the test window
    y_hat(f_idx) = mode(classes);% fost frequent number
end
...
subplot(411), 
plot(x_t), 
grid on, xlabel('sample number'), ylabel('x_t(n)'), axis tight

subplot(412), 
plot(f_t(:, 1), 'o'), 
grid on, xlabel('sample number'), ylabel('ZCR'), axis tight

subplot(413), 
plot(f_t(:, 2), 'o'), 
grid on, xlabel('sample number'), ylabel('AP'), axis tight

subplot(414), 
plot(y_hat, 'o'), grid on, xlabel('sample number'), ylabel('estimated label'), axis tight

% window by window check if the signal is voiced. In the end mb average to
% say if it is voiced or not
